...
account <- read.csv2("https://query.data.world/s/qcmry5y7pjcfks2ycqt6jk5ah5q5js", header=TRUE)
head(account)
class(account)
str(account)
...
client <- read.csv2("https://query.data.world/s/ezd5humsgednsu54moosvo5tkmb7cd", header=TRUE)
head(client)
class(client)
str(client)
...
#La variable birth_number recoge la fecha de nacimiento y el sexo, ya que para los hombres está en formato YY/MM/DD
#y para las mujeres en formato YY/MM+50/DD
...
disp <- read.csv2("https://query.data.world/s/cl4ossoe73qzvtzpyunaqxuavtzffu", header=TRUE)
head(disp)
class(disp)
...
loan <- read.csv2("https://query.data.world/s/crbem4zijzjjfwgboegstc2a3l6vy4", header=TRUE)
head(loan)
class(loan)
str(loan)
...
#Cargamos el fichero de transferencias/domiciliaciones (order) y revisamos su estructura y missings y duplicates
#Leemos el fichero de Orders
order <- read.csv2("https://query.data.world/s/dyhp7l3fljc5vidxavyc7opqphydby", header=TRUE)
head(order)
class(order)
str(order)
...
#Vamos a transferir la tipología de transferencias/domiciliaciones que tiene cada cuenta y su cantidad e importe
...
card <- read.csv2("https://query.data.world/s/roqsaj2gdjosyndvohneiukyq3injx", header=TRUE)
head(card)
class(card)
str(card)
...
district <- read.csv2("https://query.data.world/s/hsobc5rxdhqxruadbsexhqwfqahmn3", header=TRUE)
head(district)
class(district)
str(district)
...
district$A12[is.na(district$A12)] <-mean(district[district$A3==region_A12,"A12"], na.rm = TRUE)
...
district$crimes_95_ratio[position_A15]<-mean(district[district$A3==region_A15,"crimes_95_ratio"], na.rm = TRUE)
district$A15[is.na(district$A15)] <-mean(district[district$A3==region_A15,"crimes_95_ratio"], na.rm = TRUE)*district[position_A15,"A4"]
...
trans <- read.csv2("https://query.data.world/s/3gw2qs6yfgfwkkguvktj6zunpgkiv4", header=TRUE)
...
#Recodificamos las variables "type", "operation" y "k_symbol" (utilizando la información contenida en https://data.world/lpetrocelli/czech-financial-dataset-real-anonymized-transactions:
...
table(trans$type)#La categoría de "type" VYBER no está traducida, pero sólo una minoría de observaciones tienen esa categoría (16.666)
...
unique(trans[trans$operation=="Op_WithdrawalCash","type"])#Todas las Collections se correponden con Type_withdrawal y Type_VYBER de la feature type
...
#Ya hemos recopilado la información que consideramos relevante de la información disponible en https://data.world/lpetrocelli/czech-financial-dataset-real-anonymized-transactions
...
#Ahora vamos a trabajar sobre client_id_disponent y loan_id, Vamos a binarizar si una cuenta ha contratado
# préstamo (loan) o no lo ha contratado y si una cuenta tiene alguna persona como autorizado o no
account<-account%>%mutate(
...
#EN EL ANÁLISIS DEL TFM VOY A INTENTAR PREDECIR LAS CUENTAS QUE CONTRATAN UN PRÉSTAMO Y
#ESCRIBIENDO LA MEMORIA ME HE DADO CUENTA QUE POR LA VARIABLE "Sym_LoanPayment" SE ESTÁ FILTRANDO INFORMACIÓN
#DE SI UNA CUENTA TIENE PRÉSTAMO A "Num_Op_Remittances" Y "Num_Type_Withdrawal". VOY A ELIMINAR ESE EFECTO
account_wo_LoanPayment=account

hm_acc<-hashmap(account$account_id,seq(1:dim(account)[1]))
for (row_order in seq(1,dim(trans)[1])){
  row_account<-hm_acc[[trans[row_order,"account_id"]]] #Sirve el mismo diccionario
  
  if (trans[row_order,"k_symbol"]=="Sym_LoanPayment"){
    
    #Corregimos los atributos de feature Type
    if (trans[row_order,"type"]=="Type_VYBER"){
      account_wo_LoanPayment[row_account,"Num_Type_VYBER"]=account_wo_LoanPayment[row_account,"Num_Type_VYBER"]-1
    }
    if (trans[row_order,"type"]=="Type_withdrawal"){
      account_wo_LoanPayment[row_account,"Num_Type_Withdrawal"]=account_wo_LoanPayment[row_account,"Num_Type_Withdrawal"]-1
    }
    #Completamos los atributos de feature operation
    if (trans[row_order,"operation"]=="Op_Remittance"){
      account_wo_LoanPayment[row_account,"Num_Op_Remittances"]=account_wo_LoanPayment[row_account,"Num_Op_Remittances"]-1 
    }
  }
}

#vAMOS A COMPROBAR QUE ELIMINAMOS LOS EFECTOS
#Vemos cuantas transacciones de "Sym_LoanPayment" hemos transferido desde el fichero "trans"
sum(account$Num_Sym_LoanPayment)

#En Num_Type_VYBER no hay efecto 
sum(account$Num_Type_VYBER)-sum(account_wo_LoanPayment$Num_Type_VYBER)

#Todos los Sym_LoanPayment se corresponden con Num_Type_Withdrawal, por eso no hay efecto en Num_Type_VYBER
sum(account$Num_Type_Withdrawal)-sum(account_wo_LoanPayment$Num_Type_Withdrawal)

#Todos los Sym_LoanPayment se corresponden con Num_Op_Remittances, generada a partir de la variable Operation
#del fichero "trans"
sum(account$Num_Op_Remittances)-sum(account_wo_LoanPayment$Num_Op_Remittances)


setwd("C:/Master Data Science/Master en Data Science/TFM/Transacciones de Banco Checo")
write.csv(account_wo_LoanPayment, file="DFTenenciaProductos_wo_LoanPayment.csv")
